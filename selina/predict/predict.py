...
def query_predict(query_expr, model, path_out, outprefix, disease, mode):
    ...
    if disease:
            ...
            if mode=='single':
                ...
                disease_labels = pd.DataFrame({'Cluster':query_expr.columns,'Prediction':disease_labels})
            disease_labels.to_csv(path_out + '/' + outprefix +
                                        '_cellsources.txt',
                                        index=False,
                                        header=True,
                                        sep='\t')
...
def predict_downstream(mode, seurat, prob_cutoff, path_out,
                       outprefix):
    print('Begin downstream analysis')
    if mode == 'single':
        cmd = 'Rscript ' + os.path.split(
            os.path.abspath(__file__)
        )[0] + '/downstream.R ' + ' --mode ' + mode + ' --seurat ' + seurat + ' --pred_label ' + path_out + '/' + outprefix + '_predictions.txt' + ' --pred_prob ' + path_out + '/' + outprefix + '_probability.txt'  + ' --prob_cutoff ' + str(
                prob_cutoff
